...
#'
#' @param df (Dataframe) A dataframe, or something that can be coerced into a dataframe.
#' @param ... (Names) Columns names or positions that are passed to `dplyr::select()`.
#'
#' @return A dataframe.
#' @export
#'
...
howmany_df <- function(df, ...) {
    ...
    # Coerce into df if it isn't one.
    ...
    if (length(dots) == 0) {
        cols <- dplyr::select(df, dplyr::everything())
    } else {
        cols <- dplyr::select(df, !!! dots)
    }
